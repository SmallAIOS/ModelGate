# Formal methods — Design Document

> **Lineage.** The bulk of this document is the salvaged `formal-methods.md` originally written on `claude/design-smctl-tool-auqmR` for the in-flight smctl-tool-v1 proposal. That file was dropped during the merge of smctl-tool-v1 to develop. We're preserving it here, lightly edited, as the design rationale for `formal-methods-v1`. The proposal.md scopes which subset of this analysis lands in v1; the rest stays as forward-looking reference.

## Overview

`smctl verify` integrates formal methods across three verification domains: behavioural (TLA+), authorization-policy (Cedar), and theorem-proving (Lean 4). This document analyses every formal-methods tool in the SmallAIOS / ModelGate orbit — TLA+, Lean 4, Cedar, Alloy, P, SPIN/Promela, and Rego/OPA — and recommends the multi-tool strategy `formal-methods-v1` adopts.
